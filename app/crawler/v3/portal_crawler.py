...
async def board_page_crawler(session, board: str, page: int, ignore_date=False):
    ...
    url = f"https://portal.koreatech.ac.kr/ctt/bb/bulletin?b=148{board}"
    ...
    async with session.get(url, headers=headers) as resp:
        ...
        if resp.status == 200:
            ...
            posts = soup.select("#boardTypeList > tbody")

            for post in posts:
                try:
                    num_parsed = post.select_one("tr.bc-s-post-notice > td.bc-s-post_seq").text.strip()
                    article_url_parsed = post.select_one("tr.bc-s-post-notice").get('data-url')
                    article_url_parsed = f"https://portal.koreatech.ac.kr{article_url_parsed}"
                    ...
                    try:
                        importance_img = post.select_one("tr.bc-s-post-notice > td.bc-s-title > div:nth-child(1) > img")
                        if importance_img.get("src") == "/eXPortal/ctt/images/notice.gif":
                            is_importance = True
                        else:
                            is_importance = False
