...
async def remove_article(session, department: Department, board_index: int, notice_count: int, board_list=None,
    ...
    # Get notice articles
    notice_articles = client.get_articles(
        department=department.department,
        board=department.boards[board_index].board,
        is_notice=True
    )

    # Get non-notice articles with pagination
    regular_articles = client.get_articles(
        department=department.department,
        board=department.boards[board_index].board,
        is_notice=False,
        offset=(page - 1) * num_of_items,
        limit=num_of_items - notice_count
    )

    db_board_list = list(notice_articles) + list(regular_articles)
